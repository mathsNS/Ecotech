...
def aplicar(conn):
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_evento_tipo_data ON evento_operacional(tipo,criado_em)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_evento_solicitacao ON evento_operacional(solicitacao_id,criado_em)"
    )
