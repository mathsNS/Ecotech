...
def configurar_seguranca(app):
    ...
    if producao and (len(segredo_sessao) < 32 or len(segredo_jwt) < 32):
        raise RuntimeError(
            "ECOTECH_SECRET_KEY e ECOTECH_JWT_SECRET devem ter ao menos 32 caracteres em producao"
        )
...
def criar_app() -> Flask:
    ...
    servico_despacho = ServicoDespacho(
        ...
        configuracao=ConfiguracaoDespacho(
            ...
            prazo_resposta_minutos=int(os.environ.get("ECOTECH_DESPACHO_PRAZO_MINUTOS", "5")),
        ),
    )
    ...
    def criar_conta():
        ...
        if request.method == "POST":
            ...
            flash(
                f"Conta criada com sucesso! Bem-vindo(a), {usuario_obj.nome}.",
                "success",
            )
            return redirect(url_for("dashboard"))
    ...
    def dashboard():
        ...
        if usuario["tipo"] == "empresa":
            ...
            sols_finais = [
                s for s in solicitacoes_usuario if s.estado.obter_nome() in _ESTADOS_FINAIS
            ]
            sols_reciclados = [
                s for s in solicitacoes_usuario if s.estado.obter_nome() == "Reciclado"
            ]
            peso_processado = round(sum(s.calcular_peso_total() for s in sols_finais), 2)
            co2_evitado = round(peso_processado * 3.0, 1)
            taxa_reciclagem = (
                round((len(sols_reciclados) / len(sols_finais) * 100), 1) if sols_finais else 0.0
            )
            ...
            if len(meses) > 1 and meses[-1]["peso"] and meses[-2]["peso"]:
                comparativo_mensal = round(
                    (meses[-1]["peso"] - meses[-2]["peso"]) / meses[-2]["peso"] * 100, 1
                )
            em_processamento = [
                s for s in solicitacoes_usuario if s.estado.obter_nome() == "Em Processamento"
            ]
            total_ativas = sum(
                1
                for s in solicitacoes_usuario
                if s.estado.obter_nome() not in _ESTADOS_FINAIS | {"Cancelado"}
            )
        ...
        saques_realizados = sum(
            s["valor"]
            for s in servico_saque.listar_saques(usuario["id"])
            if s.get("status") != "cancelado"
        )
        saldo_acumulado = max(
            round(pontos_reais * servico_saque.TAXA_REAIS_POR_PONTO - saques_realizados, 2),
            0.0,
        )
        meta_missao = 15
        ...
        if request.method == "POST":
            try:
                ...
                for arquivo in request.files.getlist("fotos"):
                    ...
                    valida = (
                        conteudo.startswith(b"\xff\xd8\xff")
                ...
                peso_kg = (
                    float(peso_digitado)
                    if peso_informado
                    else pesos_estimados.get(tipo_dispositivo, 1.0)
                )
                ...
                data_agendamento = (
                    f"{data_coleta} {horario_coleta}".strip() if data_coleta else None
                )
                ...
                servico_descarte.adicionar_item_solicitacao(
                    solicitacao, dispositivo, quantidade, observacoes
                )
                ...
                dados.registrar_peso_estimado(solicitacao.id, peso_kg * quantidade, peso_informado)
                ...
                if tipo_coleta == "domiciliar":
                    dados.atualizar_localizacao_coleta(
                        ...
                        "navegador_ou_formulario",
                    )
                    agendada_para = (
                        datetime.strptime(data_agendamento, "%Y-%m-%d %H:%M")
                        if data_agendamento
                        else datetime.now()
                    )
                    ...
                    janela_fim = (
                        ...
                        if data_coleta and fim_informado
                        else agendada_para + timedelta(hours=2)
                    )
                    ...
                    servico_despacho.criar_ofertas(
                        solicitacao.id,
                        DemandaColeta(
                            coordenadas=Coordenadas(coordenadas.latitude, coordenadas.longitude),
    ...
    def pontos_coleta():
        ...
        return render_template("pontos_coleta.html", usuario=usuario, pontos=pontos)
                ...
                if raw_agenda:
                    for fmt in (
                        ...
                        try:
                            ...
                            data_agendamento_fmt = (
                                dt_agenda.strftime("%d/%m %H:%M")
                                if "%H" in fmt
                                else dt_agenda.strftime("%d/%m")
                            )
                            break
                        except ValueError:
                            continue
    ...
    def _dados_form_base():
        ...
        return {
            ...
            "raio_atendimento_km": request.form.get("raio_atendimento_km", "").strip(),
            "capacidade_kg": request.form.get("capacidade_kg", "").strip(),
            "realiza_coleta_domiciliar": (request.form.get("realiza_coleta_domiciliar") == "on"),
        }
    ...
    def atividade_base(id_base):
        ...
        if usuario["tipo"] != "empresa":
            return jsonify({"erro": "Acesso não autorizado"}), 403
        try:
            servico_base.definir_atividade(usuario["id"], id_base, request.form.get("ativa") == "1")
            return redirect(url_for("empresa_bases"))
        except PermissionError as exc:
            return jsonify({"erro": str(exc)}), 403
    ...
    def notificacoes():
        ...
        return render_template("notificacoes.html", usuario=usuario, notificacoes=notificacoes)
    ...
    def oportunidades_empresa():
        ...
        return render_template(
            "empresa_oportunidades.html",
            usuario=usuario,
            ofertas=todas[(pagina - 1) * limite : pagina * limite],
            pagina=pagina,
            tem_anterior=pagina > 1,
            tem_proxima=len(todas) > pagina * limite,
            bases=servico_base.listar_empresa(usuario["id"]),
        )
    ...
    def recusar_oferta(oferta_id):
        ...
        if usuario["tipo"] != "empresa":
            return jsonify({"erro": "Acesso não autorizado"}), 403
        try:
            servico_despacho.recusar(oferta_id, usuario["id"], request.form.get("motivo", ""))
    ...
    def propor_agendamento(id_sol):
        ...
        try:
            ...
            if request.form.get("_ui") == "html":
                flash("Proposta de horário enviada.", "success")
                return redirect(url_for("pagina_agendamento", id_sol=id_sol))
    ...
    def aceitar_agendamento(id_sol):
        ...
        try:
            ...
            servico_chat.evento(
                id_sol,
                "HORARIO_ACEITO",
                {"inicio": row["inicio_confirmado"], "fim": row["fim_confirmado"]},
            )
            if request.form.get("_ui") == "html":
                flash("Horário confirmado.", "success")
                return redirect(url_for("pagina_agendamento", id_sol=id_sol))
            return jsonify(
                {
                    "ok": True,
    ...
    def rejeitar_agendamento(id_sol):
        ...
        try:
            ...
            if request.form.get("_ui") == "html":
                flash("Proposta rejeitada.", "success")
                return redirect(url_for("pagina_agendamento", id_sol=id_sol))
            return jsonify({"ok": True, "status": row["status"]})
        except PermissionError as exc:
            return jsonify({"erro": str(exc)}), 403
    ...
    def chat_solicitacao(id_sol):
        ...
        try:
            ...
            if not sol or not usuario_pode_visualizar_solicitacao(usuario, sol, dados):
                return jsonify({"erro": "Acesso não autorizado"}), 403
            dados.criar_conversa_solicitacao(
                id_sol, datetime.now(timezone(timedelta(hours=-3))).isoformat()
            )
            if request.method == "POST":
    ...
    def detalhes_solicitacao(id_sol):
        ...
        fotos = [
            dict(f) | {"url": url_for("foto_solicitacao", foto_id=f["id"])}
            for f in dados.listar_fotos_solicitacao(id_sol)
        ]
        return jsonify(
            {
                "id": sol.id,
                "cidadao": sol.usuario.nome,
                "estado": sol.estado.obter_nome(),
                "criada_em": formatar_data_br(sol.data_criacao, "data_hora"),
                "peso_total": round(sol.calcular_peso_total(), 2),
                "tipo_coleta": raw["tipo_coleta"] or "Nao informado",
                "endereco": raw["endereco_coleta"] or "Nao informado",
                "contato": raw["nome_contato"] or "Nao informado",
    ...
    def foto_solicitacao(foto_id):
        ...
        usuario = dados_usuario()
        sol = servico_descarte.obter_solicitacao(foto["solicitacao_id"])
        if not sol or not usuario_pode_visualizar_solicitacao(usuario, sol, dados):
            return jsonify({"erro": "Acesso nao autorizado"}), 403
        return Response(
            foto["conteudo"],
    ...
    def saque():
        ...
        if usuario["tipo"] != "cidadao":
            flash(
                "A carteira e os saques estão disponíveis apenas para cidadãos.",
                "error",
            )
            return redirect(url_for("dashboard"))
        ...
        if request.method == "POST":
            try:
                valor_str = (
                    request.form.get("valor_saque", "0").replace("R$", "").replace(",", ".").strip()
                )
                valor = float(valor_str)
                metodo = request.form.get("metodo", "Pix")

                resultado = servico_saque.solicitar_saque(usuario["id"], valor, metodo, saldo)
    ...
    def perfil():
        ...
        _milestones = [
            (1, "1º descarte", "medalha"),
            (5, "5 descartes", "medalha"),
            (10, "10 descartes", "trofeu"),
            (20, "20 descartes", "trofeu"),
        ]
        conquistas = [
            {
                "valor": m,
                "label": lbl,
                "icone": ico,
                "ativa": len(solicitacoes_usuario) >= m,
            }
    ...
    def operacoes():
        ...
        solicitacoes_usuario = sorted(
            solicitacoes_usuario, key=lambda s: s._data_criacao, reverse=True
        )

        # filtra por estado se tiver parametro
        filtro_estado = request.args.get("estado", "")
        solicitacoes_filtradas = servico_descarte.filtrar_por_estado(
            solicitacoes_usuario, filtro_estado
        )
    ...
    def avancar_estado(id_sol):
        ...
        if usuario["tipo"] == "empresa":
            ...
            if plano == "free":
                mes_atual = datetime.now(timezone(timedelta(hours=-3))).month
                ano_atual = datetime.now(timezone(timedelta(hours=-3))).year
                solicitacoes_empresa = listar_solicitacoes_visiveis_empresa(
                    usuario["id"], todas, dados
                )
                processadas_mes = sum(
                    1
                    for s in solicitacoes_empresa
    ...
    def relatorios():
        ...
        try:
            ...
            if data_fim_str:
                data_fim_dt = datetime.strptime(data_fim_str, "%Y-%m-%d").replace(
        ...
        solicitacoes_finalizadas = [
            s
            for s in solicitacoes_para_relatorio
        ...
        plano_empresa = (
            dados.buscar_plano_empresa(usuario["id"]) if usuario["tipo"] == "empresa" else None
        )
    ...
    def exportar_csv():
        ...
        if usuario["tipo"] == "empresa":
            ...
            if plano == "free":
                flash(
                    "Exportação de dados disponível apenas nos planos Professional e Enterprise.",
                    "error",
                )
    ...
    def gerar_mtr_pdf(id_sol):
        ...
        if usuario["tipo"] == "empresa":
            ...
            if plano == "free":
                flash(
                    "Geração de MTR disponível apenas nos planos Professional e Enterprise.",
                    "error",
                )
                return redirect(url_for("operacoes"))
        ...
        base_mtr = (
            ...
            if registro["base_operacional_id"]
            else None
        )
    ...
    def usuarios():
        ...
        cidadaos = [{**c, "data_cadastro": formatar_data(c["data_cadastro"])} for c in cidadaos_raw]
        empresas = [{**e, "data_cadastro": formatar_data(e["data_cadastro"])} for e in empresas_raw]
    ...
    def api_solicitacoes():
        ...
        if usuario["tipo"] == "administrador":
            ...
            lista = listar_solicitacoes_visiveis_empresa(usuario["id"], todas, dados)
        else:
        ...
        pendentes = [
            {
                ...
                "estado_produto": _estado_labels.get(
                    row["estado_produto"], row["estado_produto"] or "-"
                ),
    ...
    def atualizar_preco(subcategoria):
        ...
        if valor_base <= 0 or valor_minimo < 0:
            flash(
                "Valor base deve ser positivo e valor mínimo não pode ser negativo.",
                "error",
            )
            return redirect(url_for("admin_precos"))
    ...
    def processar_ofertas(agora):
        ...
        instante = datetime.fromisoformat(agora) if agora else datetime.now(timezone(timedelta(hours=-3)))
...
def _inicializar_dados_exemplo(servico_usuario, servico_ponto, servico_descarte, dados):
    ...
    cidadao1 = servico_usuario.criar_usuario(
        "cidadao",
        {
            "id": "user-1",
            "nome": "João Silva",
    ...
    empresa1 = servico_usuario.criar_usuario(
        "empresa",
        {
            "id": "user-2",
            "nome": "Recicla Kariri",
    ...
    # ---- cidadãos adicionais ----
    cidadao2 = servico_usuario.criar_usuario(
        "cidadao",
        {
            "id": "user-3",
    ...
    cidadao3 = servico_usuario.criar_usuario(
        "cidadao",
        {
            "id": "user-4",
    ...
    cidadao4 = servico_usuario.criar_usuario(
        "cidadao",
        {
            "id": "user-5",
            "nome": "Fernanda Lima",
    ...
    cidadao5 = servico_usuario.criar_usuario(
        "cidadao",
        {
            "id": "user-6",
            "nome": "Rafael Gonçalves",
            "email": "rafael.goncalves@hotmail.com",
            "cpf": "70548478490",
        },
        senha="rafael123",
    )
    ...
    # ---- empresas adicionais ----
    empresa2 = servico_usuario.criar_usuario(
        "empresa",
        {
            "id": "user-7",
            "nome": "TechLixo Soluções",
            "email": "contato@techlixo.com.br",
            "cnpj": "14380200000121",
            "razao_social": "TechLixo Soluções Ambientais LTDA",
        },
        senha="techlixo123",
    )
    empresa3 = servico_usuario.criar_usuario(
        "empresa",
        {
            "id": "user-8",
            "nome": "GreenCycle Nordeste",
            "email": "admin@greencycle.com.br",
            "cnpj": "33000167000101",
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol1,
        DispositivoFactory.criar_celular("cel-001", "iPhone 11", 0.194),
        1,
        "tela quebrada",
    )
    servico_descarte.adicionar_item_solicitacao(
        sol1,
        DispositivoFactory.criar_computador("comp-001", "Dell Inspiron 15 3000", 2.1),
        1,
        "não liga mais",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol2,
        DispositivoFactory.criar_eletrodomestico("elet-001", 'TV Samsung 32" Smart', 5.5),
        1,
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol3,
        DispositivoFactory.criar_computador("comp-002", 'Monitor LG 24" Full HD', 3.2),
        1,
    )
    servico_descarte.adicionar_item_solicitacao(
        sol3,
        DispositivoFactory.criar_computador("comp-003", "Teclado Mecânico Redragon", 0.8),
        1,
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol4,
        DispositivoFactory.criar_eletrodomestico(
            "elet-002", "Geladeira Brastemp Frost Free 360L", 45.0
        ),
        1,
        "compressor queimado",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol5,
        DispositivoFactory.criar_computador("comp-004", "Impressora HP LaserJet Pro M404n", 6.2),
        2,
        "cartucho vazando",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol6,
        DispositivoFactory.criar_celular("cel-002", "Samsung Galaxy A52", 0.189),
        1,
    )
    servico_descarte.adicionar_item_solicitacao(
        sol6,
        DispositivoFactory.criar_celular("cel-003", "Xiaomi Redmi Note 11", 0.179),
        1,
        "bateria inchada",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol7,
        DispositivoFactory.criar_eletrodomestico("elet-003", "Air Fryer Mondial 4L", 3.8),
        1,
        "resistência queimada",
    )
    servico_descarte.adicionar_item_solicitacao(
        sol7,
        DispositivoFactory.criar_eletrodomestico("elet-004", "Liquidificador Arno Clic Pro", 1.6),
        1,
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_rk1,
        DispositivoFactory.criar_celular("cel-004", "Samsung Galaxy S10", 0.175),
        3,
    )
    servico_descarte.adicionar_item_solicitacao(
        sol_rk1,
        DispositivoFactory.criar_celular("cel-005", "iPad Air 3ª geração", 0.460),
        2,
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_rk2,
        DispositivoFactory.criar_computador("comp-005", "Desktop HP EliteDesk 800 G5", 8.5),
        5,
        "lote corporativo",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_rk3,
        DispositivoFactory.criar_computador("comp-006", "Servidor Dell PowerEdge R440", 12.0),
        2,
        "substituídos por novo modelo",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_rk4,
        DispositivoFactory.criar_eletrodomestico("elet-005", "Ar Condicionado Gree 12000BTU", 28.5),
        4,
        "gás refrigerante obsoleto",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_ana1,
        DispositivoFactory.criar_celular("cel-006", "Motorola Moto G82", 0.173),
        1,
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_ana2,
        DispositivoFactory.criar_eletrodomestico("elet-006", "Micro-ondas Electrolux MEF41", 11.0),
        1,
        "prato giratório quebrado",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_ana3,
        DispositivoFactory.criar_celular("cel-007", "Notebook Samsung Book", 1.5),
        1,
        "tela com manchas",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_car1,
        DispositivoFactory.criar_computador("comp-007", "Notebook Lenovo ThinkPad E14", 1.85),
        1,
    )
    servico_descarte.adicionar_item_solicitacao(
        sol_car1,
        DispositivoFactory.criar_computador("comp-008", "Mouse sem fio Logitech MX Master", 0.09),
        2,
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_car2,
        DispositivoFactory.criar_eletrodomestico("elet-007", "Micro System Philips BTB2595", 4.5),
        1,
        "caixa de som danificada",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_fer1,
        DispositivoFactory.criar_celular("cel-008", "Samsung Galaxy Tab A7", 0.476),
        1,
    )
    servico_descarte.adicionar_item_solicitacao(
        sol_fer1,
        DispositivoFactory.criar_celular("cel-009", "AirPods Pro 2ª geração", 0.061),
        2,
        "bateria viciada",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_fer2,
        DispositivoFactory.criar_eletrodomestico("elet-008", "Cafeteira Nespresso Vertuo", 2.3),
        1,
        "bomba de pressão com defeito",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_raf1,
        DispositivoFactory.criar_computador("comp-009", "PC Gamer Pichau Orion", 9.2),
        1,
        "fonte queimada",
    )
    servico_descarte.adicionar_item_solicitacao(
        sol_raf1,
        DispositivoFactory.criar_celular("cel-010", 'Monitor Gamer ASUS 27" 144Hz', 4.8),
        1,
        "pixels mortos",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_tl1,
        DispositivoFactory.criar_computador("comp-010", "Switch Cisco Catalyst 2960", 4.5),
        4,
        "fim de vida útil",
    )
    servico_descarte.adicionar_item_solicitacao(
        sol_tl1,
        DispositivoFactory.criar_computador("comp-011", "Roteador Mikrotik CCR1009", 1.2),
        6,
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_tl2,
        DispositivoFactory.criar_eletrodomestico("elet-009", "Nobreak APC Smart-UPS 1500VA", 22.0),
        8,
        "baterias sulfatadas",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_tl3,
        DispositivoFactory.criar_computador("comp-012", "Workstation Dell Precision 5820", 18.0),
        3,
        "atualização de parque",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_gc1,
        DispositivoFactory.criar_eletrodomestico("elet-010", 'TV LG 55" OLED C1', 17.8),
        3,
        "telas com defeito de fábrica",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_gc2,
        DispositivoFactory.criar_eletrodomestico(
            "elet-011", "Ar Condicionado Daikin Inverter 18000BTU", 35.0
        ),
        2,
        "gás R22 obsoleto",
    )
    ...
    servico_descarte.adicionar_item_solicitacao(
        sol_gc3,
        DispositivoFactory.criar_computador(
            "comp-013", "Laptop Corporativo Lenovo ThinkPad T490", 1.6
        ),
        12,
        "renovação de frota",
    )
    ...
    _notifs = [
        (
            "user-1",
            "Sua solicitação foi recebida e aguarda coleta. Em breve entraremos em contato.",
        ),
        (
            "user-1",
            "Parabéns! Você ganhou R$ 40,20 de incentivo pela coleta ENT-2026-0412.",
        ),
        ("user-1", 'Sua TV Samsung 32" foi coletada e está em processamento.'),
        (
            "user-1",
            "Missão concluída: você atingiu 5 descartes realizados! Continue assim.",
        ),
        (
            "user-1",
            "Novo bônus disponível: ganhe 20% a mais neste fim de semana em coletas de eletrodomésticos.",
        ),
        (
            "user-1",
            "Sua solicitação de saque foi processada. Valor creditado em até 2 dias úteis.",
        ),
        (
            "user-2",
            "Lote de computadores Dell foi recebido no Ecoponto Centro. Processamento iniciado.",
        ),
        ...
        (
            "user-3",
            "Micro-ondas processado com sucesso. R$ 1,10 de incentivo creditado.",
        ),
        ("user-4", "Coleta do Micro System Philips agendada. Aguarde confirmação."),
        (
            "user-4",
            "Reciclagem concluída! Você evitou 0,5 kg de CO₂ com este descarte.",
        ),
        ...
        (
            "user-6",
            "PC Gamer e monitor retirados para reutilização. Obrigado pelo descarte responsável!",
        ),
        (
            "user-7",
            "Lote de switches Cisco foi reciclado. Relatório de impacto disponível.",
        ),
        ("user-7", "Novo ponto de coleta disponível próximo à sua área de operação."),
        (
            "user-8",
            "Renovação de frota aprovada: 12 notebooks ThinkPad encaminhados para reuso.",
        ),
        (
            "user-8",
            "Ar condicionados coletados e em processo de reutilização de componentes.",
        ),
    ]
