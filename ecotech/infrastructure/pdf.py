...
def gerar_mtr(sol) -> bytes:
    ...
    documento_gerador = (
        getattr(sol.usuario, "cpf", None) or getattr(sol.usuario, "cnpj", None) or "Nao cadastrado"
    )
    ...
    _campo_duplo(
        pdf,
        "Origem",
        "Residencial" if "cidad" in tipo_gerador else "Comercial",
        "Estado fisico",
        "Solido",
    )
    _campo_duplo(
        pdf,
        "Acondicionamento",
        "Volumes / embalagens adequadas",
        "Quantidade de volumes",
        str(sum(i.quantidade for i in sol.itens)),
    )
    _campo_duplo(
        pdf,
        "Peso declarado",
        f"{sol.calcular_peso_total():.3f} kg",
        "Situacao do peso",
        "Aferido"
        if getattr(sol, "peso_confirmado_kg", None) is not None
        else "Estimado - afericao pendente",
    )
    ...
    _campo_duplo(
        pdf,
        "Contato",
        registro.get("nome_contato") or sol.usuario.nome,
        "E-mail",
        getattr(sol.usuario, "email", "Nao cadastrado"),
    )
    ...
    for i, item in enumerate(sol.itens):
        ...
        classe = _classe_residuo(
            getattr(item.dispositivo, "categoria", None)
            or getattr(item.dispositivo, "tipo", None)
            or nome_disp
        )
    ...
    _campo(
        ...
        f"{sol.calcular_impacto_total():.2f} kg CO2 evitado",
    )
    ...
    razao_empresa = (
        ...
        or empresa.get("nome")
        or (sol.ponto_coleta.nome if sol.ponto_coleta else "Nao definido")
    )
    cnpj_empresa = empresa.get("cnpj") or "Nao cadastrado"
    endereco_operacional = base.get("endereco") or (
        sol.ponto_coleta.endereco if sol.ponto_coleta else "Nao cadastrado"
    )
    _caixa_parte(
        pdf,
        "TRANSPORTADOR",
        ...
        empresa.get("email") or "Nao cadastrado",
        "Motorista: ____________________  Placa: ______________",
    )
    _caixa_parte(
        pdf,
        "RECEPTOR / DESTINADOR",
        ...
        endereco_operacional,
        empresa.get("email") or "Nao cadastrado",
        "Licenca ambiental: ______________________________",
    )
    ...
    pdf.cell(
        0,
        5,
        f"Documento gerado automaticamente pelo sistema EcoTech em {data_emissao}. Numero: {numero_mtr}",
        align="C",
    )
...
def _campo_duplo(pdf: FPDF, rotulo_1: str, valor_1: str, rotulo_2: str, valor_2: str):
    ...
    for inicio, rotulo, valor, largura in (
        (x + 2, rotulo_1, valor_1, 88),
        (x + 94, rotulo_2, valor_2, 90),
    ):
