...
class ServicoDespacho:
    ...
    def criar_ofertas(
        ...
        if not ofertas:
            self._dados.marcar_despacho_esgotado(solicitacao_id, self._iso(agora))
            self._dados.registrar_evento_operacional(
                "SEM_EMPRESA_ELEGIVEL",
                solicitacao_id,
    ...
    def aceitar(
        ...
        agora = agora or datetime.now(timezone(timedelta(hours=-3)))
        try:
            return self._dados.aceitar_oferta_coleta(oferta_id, empresa_id, self._iso(agora))
        except RuntimeError:
            self._dados.registrar_evento_operacional(
                "CONFLITO_ACEITE",
                oferta_id=oferta_id,
    ...
    def recusar(self, oferta_id, empresa_id, motivo="", agora=None):
        agora = agora or datetime.now(timezone(timedelta(hours=-3)))
        ativas = self._dados.recusar_oferta_coleta(
            oferta_id,
            empresa_id,
            self._iso(agora),
            self._iso(self._expiracao(agora)),
            motivo,
        )
        self._notificar_ativas(ativas)
        return ativas
