...
from ..domain.descarte import ItemDescarte, PontoColeta, SolicitacaoDescarte
...
class ServicoDescarte:
    ...
    def __init__(
        self,
        dados: RepositorioBase | None = None,
    ...
    def _carregar_solicitacoes_do_banco(self):
        ...
        for row in solicitacoes_db:
            try:
                ...
                ponto = (
                    self._servico_ponto.buscar_ponto(row["id_ponto_coleta"])
                    if row["id_ponto_coleta"]
                    else None
                )
                ...
                if row["data_criacao"]:
                    try:
                        solicitacao._data_criacao = datetime.strptime(
                            row["data_criacao"], "%d/%m/%Y %H:%M"
                        )
                ...
                if row["metodo_tratamento"]:
                    ...
                    if tipo_metodo:
                        solicitacao.metodo_tratamento = MetodoTratamentoFactory.criar_metodo(
                            tipo_metodo
                        )
    ...
    def criar_solicitacao(
        self, usuario: Usuario, ponto_coleta: PontoColeta | None = None
    ) -> SolicitacaoDescarte:
    ...
    def definir_ponto_coleta(self, solicitacao: SolicitacaoDescarte, ponto_coleta: PontoColeta):
        ...
        if not ponto_coleta.pode_receber(peso_total):
            raise ValueError(f"ponto de coleta {ponto_coleta.nome} nao tem capacidade")
    ...
    def definir_metodo_tratamento(self, solicitacao: SolicitacaoDescarte, metodo: MetodoTratamento):
    ...
    def obter_solicitacao(self, id: str) -> SolicitacaoDescarte | None:
    ...
    def validar_override(
        valor_proposto: float, valor_base: float, valor_minimo_sucata: float
    ) -> dict:
        ...
        return {
            "status": "aprovado",
            "motivo": "",
            "valor_aplicado": round(valor_proposto, 2),
        }
    ...
    def filtrar_por_estado(
        solicitacoes: list[SolicitacaoDescarte], estado: str = ""
    ...
    def calcular_stats_estados(solicitacoes: list[SolicitacaoDescarte]) -> dict:
        ...
        return {
            ...
            "processando": sum(
                1 for s in solicitacoes if s.estado.obter_nome() == "Em Processamento"
            ),
...
class ServicoSaque:
    ...
    def __init__(self, dados: RepositorioBase | None = None):
        self._dados = dados
    ...
    def calcular_pontos(self, solicitacoes: list[SolicitacaoDescarte], id_usuario: str) -> int:
        ...
        peso_total = sum(
            s.calcular_peso_total() for s in solicitacoes if s.usuario.id == id_usuario
        )
        return int(peso_total * self.TAXA_PONTOS_POR_KG)

    def calcular_saldo_disponivel(
        self, solicitacoes: list[SolicitacaoDescarte], id_usuario: str
    ) -> float:
        """
    ...
    def solicitar_saque(
        ...
        if chave and self._dados:
            existente = next(
                (
                    dict(item)
                    for item in self._dados.buscar_saques_usuario(id_usuario)
                    if item["id"] == id_saque
                ),
                None,
            )
            if existente:
                if (
                    ...
                    raise ValueError("A chave de idempotencia ja foi usada com outros dados.")
        ...
        if self._dados:
            self._dados.salvar_saque(id_saque, id_usuario, valor, metodo, data, hora, "pendente")
...
class ServicoAutenticacao:
    ...
    def autenticar(self, tipo: str, credencial: str, senha: str) -> Usuario | None:
        """
...
class ServicoPontoColeta:
    ...
    def buscar_ponto(self, id: str) -> PontoColeta | None:
    ...
    def definir_atividade_da_empresa(
        ...
        if not row or row["id_empresa"] != empresa_id:
            raise PermissionError("ponto de coleta nao pertence a empresa")
        self._dados.definir_atividade_ponto_empresa(id_ponto, empresa_id, ativo)
...
class ServicoBaseOperacional:
    ...
    def _de_row(row) -> BaseOperacional:
        ...
        if "janelas" in chaves and row["janelas"]:
            for janela in row["janelas"].split(","):
                ...
                janelas.append(
                    JanelaAtendimento(int(dia), time.fromisoformat(inicio), time.fromisoformat(fim))
                )
        ...
        return BaseOperacional(
            ...
            ponto_coleta_id=row["ponto_coleta_id"],
            ...
            capacidade_comprometida_kg=(
                row["capacidade_comprometida_kg"] if "capacidade_comprometida_kg" in chaves else 0
            ),
        )
    ...
    def criar(self, empresa_id: str, dados_base: dict) -> BaseOperacional:
        base = BaseOperacional(
            ...
            realiza_coleta_domiciliar=bool(dados_base.get("realiza_coleta_domiciliar", True)),
        )
        self._dados.salvar_base_operacional(base)
        return base

    def listar_empresa(self, empresa_id: str) -> list[BaseOperacional]:
        return [self._de_row(row) for row in self._dados.buscar_bases_empresa(empresa_id)]

    def buscar(self, id_base: str) -> BaseOperacional | None:
        row = self._dados.buscar_base_operacional(id_base)
        return self._de_row(row) if row else None

    def atualizar(self, empresa_id: str, id_base: str, dados_base: dict) -> BaseOperacional:
        ...
        base = BaseOperacional(
            ...
            realiza_coleta_domiciliar=bool(dados_base.get("realiza_coleta_domiciliar", False)),
            ativa=atual.ativa,
            ponto_coleta_id=atual.ponto_coleta_id,
        )
        self._dados.atualizar_base_operacional(base)
        return base

    def definir_atividade(self, empresa_id: str, id_base: str, ativa: bool) -> None:
        atual = self.buscar(id_base)
        if not atual or not atual.pertence_a(empresa_id):
            raise PermissionError("base operacional não pertence à empresa")
    ...
    def configurar_categorias(self, empresa_id: str, id_base: str, categorias: list[str]) -> None:
    ...
    def definir_indisponibilidade(self, empresa_id: str, id_base: str, indisponivel_ate) -> None:
        self._dados.definir_indisponibilidade_base(id_base, empresa_id, indisponivel_ate)
...
class ServicoUsuario:
        ...
        for u in usuarios_db:
            try:
                if u["tipo"] == "cidadao":
                    ...
                    usuario = Administrador(u["id"], u["nome"], u["email"])
                    self._usuarios[u["id"]] = usuario
    ...
    def buscar_usuario(self, id: str) -> Usuario | None:
