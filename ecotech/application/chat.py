...
class ServicoChat:
    ...
    def enviar(self,solicitacao_id,usuario_id,texto,agora=None,chave_idempotencia=None):
        ...
        if chave_idempotencia is not None:
            ...
            id_mensagem = str(uuid.uuid5(
                uuid.NAMESPACE_URL,
                f'ecotech:{solicitacao_id}:{usuario_id}:{chave_idempotencia}',
            ))
        else:
            id_mensagem = str(uuid.uuid4())
        return self._dados.salvar_mensagem_chat(id_mensagem,solicitacao_id,usuario_id,'MENSAGEM',texto,'{}',(agora or datetime.now()).isoformat())
    ...
    def listar_recentes(self,solicitacao_id,usuario_id,pagina=1,limite=50):
        return self._dados.buscar_mensagens_chat_recentes(solicitacao_id,usuario_id,max(1,pagina),min(max(1,limite),100))
