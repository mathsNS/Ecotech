...
class ServicoRanking:
    ...
    def ordenar(self, candidatas: Iterable[BaseElegivel]) -> list[ResultadoRanking]:
        ...
        for candidata in candidatas:
            base = candidata.base
            fator_distancia = 1 - min(candidata.distancia_km / base.raio_atendimento_km, 1)
            fator_capacidade = min(base.capacidade_disponivel_kg / base.capacidade_kg, 1)
            ...
            score = sum(fatores[nome] * peso for nome, peso in asdict(self._pesos).items()) / sum(
                asdict(self._pesos).values()
            )
            snapshot = {
