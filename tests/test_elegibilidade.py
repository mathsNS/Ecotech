...
def demanda(**mudancas):
    dados = dict(
        coordenadas=Coordenadas(0, 0.05),
        categorias=frozenset({"celular"}),
        peso_kg=5,
...
def test_base_elegivel_retorna_distancia_explicavel():
    ...
    assert resultado[0].distancia_km == pytest.approx(5.56, abs=0.2)
...
def test_categoria_curinga_atende_demanda_com_varias_categorias():
    resultado = ServicoElegibilidade().selecionar(
        ...
        demanda(categorias=frozenset({"celular", "notebook"})),
        AGENDAMENTO,
    )
