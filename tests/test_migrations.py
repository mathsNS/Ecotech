...
def test_banco_vazio_sobe_na_versao_atual(dados):
    assert dados.buscar_versao_schema() == 13
...
def test_reexecutar_migrations_e_idempotente(dados):
    ...
    executar_migrations(dados.conn)
    total = dados.conn.execute("SELECT COUNT(*) FROM schema_migration").fetchone()[0]
    assert total == 13
...
def test_migration_cria_estruturas_de_elegibilidade(dados):
    tabelas = {
        row["name"]
        for row in dados.conn.execute("SELECT name FROM sqlite_master WHERE type = 'table'")
    }
    assert {"base_categoria", "base_disponibilidade"} <= tabelas
    colunas_base = {
        row["name"] for row in dados.conn.execute("PRAGMA table_info(base_operacional)")
    }
    colunas_solicitacao = {
        row["name"] for row in dados.conn.execute("PRAGMA table_info(solicitacao_descarte)")
    }
...
def test_migration_cria_ofertas_e_idempotencia_de_notificacao(dados):
    ...
    assert "lida_em" in colunas_notificacao
    colunas_solicitacao = {
        row["name"] for row in dados.conn.execute("PRAGMA table_info(solicitacao_descarte)")
    }
...
def test_colunas_legadas_sao_adicionadas_sem_ocultar_erros(tmp_path, monkeypatch):
    ...
    colunas = {row["name"] for row in repositorio.conn.execute("PRAGMA table_info(usuario)")}
    assert "password_hash" in colunas
    assert repositorio.buscar_versao_schema() == 13


def test_migration_adiciona_ano_de_fabricacao_ao_dispositivo(dados):
    colunas = {row["name"] for row in dados.conn.execute("PRAGMA table_info(dispositivo)")}
    assert "ano_fabricacao" in colunas


def test_migration_cria_base_para_ponto_empresarial(dados):
    empresa = Empresa(
        "emp-base", "Empresa Base", "base@teste.com", "11222333000181", "Empresa Base LTDA"
    )
...
def test_entrega_e_receita_sao_idempotentes_por_solicitacao(dados):
    ...
    empresa = Empresa("emp-1", "Empresa Teste", "emp@teste.com", "11222333000181", "Empresa LTDA")
...
def test_recarregar_solicitacao_reconstroi_tratamento_e_agendamento(dados):
    ...
    dados.salvar_solicitacao(solicitacao)
    dados.atualizar_detalhes_coleta(solicitacao.id, "entrega_ponto", "", "", "2026-09-10 14:30")
