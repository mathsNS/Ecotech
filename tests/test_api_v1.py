...
def test_login_cidadao_com_credenciais_validas(client):
    resp = client.post(
        ...
        json={
            "tipo": "cidadao",
            "credencial": _CPF_CIDADAO,
            "senha": _SENHA_CIDADAO,
        },
    )
    corpo = resp.get_json()
...
def test_login_empresa_com_credenciais_validas(client):
    resp = client.post(
        ...
        json={
            "tipo": "empresa",
            "credencial": _CNPJ_EMPRESA,
            "senha": _SENHA_EMPRESA,
        },
    )
    corpo = resp.get_json()
    assert resp.status_code == 200
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            _CNPJ_EMPRESA,
            _SENHA_EMPRESA,
            {"finalizadas", "ativas", "peso_processado_kg", "saldo"},
        ),
        (
            "administrador",
            _EMAIL_ADMIN,
            _SENHA_ADMIN,
            {"peso_total_kg", "solicitacoes", "finalizadas", "receita"},
        ),
    ],
)
def test_dashboard_retorna_contrato_especifico_por_perfil(
    ...
    if tipo == "empresa":
        assert all("base_operacional" in solicitacao for solicitacao in corpo["em_processamento"])
...
def test_solicitacao_de_outro_cidadao_nao_pode_ser_acessada(client):
    ...
    cadastro = client.post(
        ...
        json={
            "tipo": "cidadao",
            "nome": "Outro Cidadao",
            "email": "outro.cidadao.mobile@example.com",
            "senha": "senha123",
            "senha_confirmacao": "senha123",
            "cpf": "16899535009",
        },
...
def test_empresa_cria_edita_e_desativa_base_sem_coordenadas_na_api(
    ...
    editada = client.patch(f"/api/v1/empresa/bases/{base['id']}", headers=headers, json=payload)
    ...
    invasao = client.patch(f"/api/v1/empresa/bases/{base['id']}", headers=outra, json=payload)
...
def test_empresa_gerencia_ponto_e_confirma_entrega_com_peso_aferido(
    ...
    headers_cidadao = {
        "Authorization": "Bearer " + _obter_token(client, "cidadao", _CPF_CIDADAO, _SENHA_CIDADAO)
    }
    solicitacao = client.post(
        "/api/v1/solicitacoes",
        headers=headers_cidadao,
    ...
    ponto_atual = next(item for item in pontos.get_json()["pontos"] if item["id"] == ponto["id"])
    entrega = next(item for item in ponto_atual["solicitacoes"] if item["id"] == solicitacao_id)
    assert entrega["pode_confirmar"] is True
    assert entrega["peso_confirmado_kg"] is None

    confirmada = client.post(
        f"/api/v1/empresa/pontos/{ponto['id']}/solicitacoes/{solicitacao_id}/confirmar",
        headers=headers_empresa,
        json={"peso_kg": "0,55"},
    )
    ...
    desativado = client.delete(f"/api/v1/empresa/pontos/{ponto['id']}", headers=headers_empresa)
...
def test_endpoints_empresariais_bloqueiam_outros_perfis(client):
    ...
    assert client.get("/api/v1/empresa/oportunidades", headers=headers).status_code == 403
...
def test_oportunidade_preserva_privacidade_e_aceite_e_atomico(client):
    ...
    assert lista.status_code == 200
    oferta = next(
        item for item in lista.get_json()["oportunidades"] if item["id"] == "oferta-api-recicla"
    )
    ...
    conflito = client.post("/api/v1/empresa/oportunidades/oferta-api-tech/aceitar", headers=tech)
    ...
    recusada = client.post(
        ...
        headers=recicla,
        json={"motivo": "Sem veiculo disponivel"},
    )
    assert recusada.status_code == 200
    assert (
        db.conn.execute("SELECT status FROM oferta_coleta WHERE id='oferta-api-recusa'").fetchone()[
            "status"
        ]
        == "RECUSADA"
    )
...
def test_operacao_exige_peso_avalia_produto_e_gera_mtr(client):
    ...
    admin = {
        ...
        + _obter_token(client, "administrador", _EMAIL_ADMIN, _SENHA_ADMIN)
    }
    mtr = client.get(f"/api/v1/operacoes/{solicitacao_id}/mtr", headers=admin)
...
def test_operacoes_bloqueiam_cidadao_e_isolam_empresas(client):
    ...
    operacao_recicla = next(
        item for item in operacoes["operacoes"] if item["empresa"] == "Recicla Kariri"
    )
    tech = _cabecalho_empresa(client, "14380200000121", "techlixo123")
    negado = client.get(f"/api/v1/operacoes/{operacao_recicla['id']}", headers=tech)
...
def test_agenda_chat_notificacoes_e_badges_mobile(client):
    ...
    agenda = client.get(f"/api/v1/solicitacoes/{solicitacao_id}/agendamento", headers=empresa)
    ...
    conversa = next(
        item
        for item in conversa_empresa.get_json()["conversas"]
    ...
    primeira = client.post(
        ...
        headers=cidadao,
        json={"texto": texto, "id_cliente": "msg-mobile-1"},
    )
    ...
    mensagem = next(item for item in mensagens if item["id"] == primeira.get_json()["id"])
    ...
    tech = _cabecalho_empresa(client, "14380200000121", "techlixo123")
    terceiro = client.get(f"/api/v1/conversas/{solicitacao_id}/mensagens", headers=tech)
    assert terceiro.status_code == 403
    badges = client.get("/api/v1/badges", headers=empresa).get_json()
    assert badges["mensagens"] >= 1
    leitura = client.post(f"/api/v1/conversas/{solicitacao_id}/leitura", headers=empresa)
...
def test_carteira_e_saque_idempotente_mobile(client):
    ...
    assert primeiro.get_json()["carteira"]["saldo"] == pytest.approx(saldo_inicial - 10)
...
def test_relatorios_respeitam_escopo_periodo_plano_e_csv(client):
    ...
    assert all(
        item["estado"] in ("Reciclado", "Reutilizado", "Descartado")
        for item in corpo["finalizadas"]
    )
    assert client.get("/api/v1/relatorios/exportar.csv", headers=recicla).status_code == 403

    db.atualizar_plano_empresa("user-2", "professional")
    csv_response = client.get("/api/v1/relatorios/exportar.csv", headers=recicla)
...
def test_admin_usuarios_cadastro_busca_paginacao_e_soft_delete(client):
    ...
    filtrado = client.get("/api/v1/admin/usuarios?busca=administrado", headers=admin).get_json()
    assert filtrado["paginacao"]["total"] == 1
    assert filtrado["usuarios"][0]["ativo"] is True

    desativado = client.post(f"/api/v1/admin/usuarios/{usuario_id}/desativar", headers=admin)
    repetido = client.post(f"/api/v1/admin/usuarios/{usuario_id}/desativar", headers=admin)
    assert desativado.status_code == 200
    assert repetido.get_json()["repetido"] is True
    preservado = client.get("/api/v1/admin/usuarios?busca=administrado", headers=admin).get_json()[
        "usuarios"
    ][0]
...
def test_admin_despacho_expoe_destinatarios_reais(client):
    ...
    assert all(item["empresa_nome"] and item["base_nome"] for item in corpo["destinatarios"])
...
def test_admin_override_decisao_idempotente_e_notifica_empresa(client):
    ...
    item = next(
        item for item in fila.get_json()["overrides"] if item["solicitacao_id"] == solicitacao["id"]
    )
...
def test_admin_precos_valida_edicao_e_bloqueia_nao_admin(client):
    ...
    atual = next(
        item for item in tabela.get_json()["precos"] if item["subcategoria"] == "smartphone_basico"
    )
    ...
    atualizado = next(
        item
        for item in alterado.get_json()["precos"]
...
def test_planos_expoe_catalogo_limites_e_feature_flags(client):
    ...
    assert [item["id"] for item in corpo["planos"]] == ["free", "professional", "enterprise"]
    ...
    assert corpo["feature_flags"] == next(
        item["feature_flags"] for item in corpo["planos"] if item["id"] == corpo["plano_atual"]
    )


def test_alteracao_de_plano_valida_perfil_e_e_idempotente(client):
    empresa = _cabecalho_empresa(client)
    plano_original = client.get("/api/v1/planos", headers=empresa).get_json()["plano_atual"]
    destino = "enterprise" if plano_original != "enterprise" else "professional"
    try:
        alterado = client.post("/api/v1/planos/alterar", headers=empresa, json={"plano": destino})
        repetido = client.post("/api/v1/planos/alterar", headers=empresa, json={"plano": destino})
        invalido = client.post("/api/v1/planos/alterar", headers=empresa, json={"plano": "gold"})
        assert alterado.status_code == 200
