...
from tests.test_despacho import AGORA, _preparar
...
def preparar(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError):
        chat.criar_para_atribuicao("sol-1", AGORA)
...
def test_encerramento_preserva_historico_e_bloqueia_envio(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError):
        chat.enviar("sol-1", "cid-1", "Depois", AGORA)
...
def test_envio_idempotente_e_paginacao_pelas_mais_recentes(tmp_path, monkeypatch):
    dados, chat = preparar(tmp_path, monkeypatch)
    primeira = chat.enviar("sol-1", "cid-1", "Mensagem unica", AGORA, chave_idempotencia="mobile-1")
    repetida = chat.enviar("sol-1", "cid-1", "Mensagem unica", AGORA, chave_idempotencia="mobile-1")
