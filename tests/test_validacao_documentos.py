...
from ecotech.application.services import ServicoUsuario, _validar_cnpj, _validar_cpf
...
def test_criar_cidadao_cpf_invalido_levanta_valueerror(servico):
    with pytest.raises(ValueError, match="CPF inválido"):
        servico.criar_usuario(
            "cidadao", {"nome": "Teste", "email": "t@t.com", "cpf": "11111111111"}, senha="senha123"
        )


def test_criar_cidadao_cpf_valido_cria_usuario(servico):
    u = servico.criar_usuario(
        "cidadao",
...
def test_criar_empresa_cnpj_valido_cria_usuario(servico):
    u = servico.criar_usuario(
        "empresa",
        {
            ...
            "email": "rk@t.com",
            "cnpj": "11222333000181",
            "razao_social": "Recicla Kariri LTDA",
        },
        senha="senha123",
    )
    assert u is not None
