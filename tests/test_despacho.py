...
def _preparar(tmp_path, monkeypatch, lotes=(1, 2)):
    ...
    for indice, (longitude, cnpj) in enumerate(zip((0.01, 0.02, 0.03), cnpjs), 1):
        empresa = Empresa(
            f"emp-{indice}",
            f"Empresa {indice}",
            f"e{indice}@teste.com",
            cnpj,
            f"Empresa {indice} LTDA",
        )
...
def test_esgotamento_mantem_solicitacao_recuperavel(tmp_path, monkeypatch):
    ...
    assert all(o["status"] == "EXPIRADA" for o in dados.buscar_ofertas_solicitacao("sol-1"))
...
def test_base_nao_elegivel_nao_recebe_oferta(tmp_path, monkeypatch):
    ...
    servico.criar_ofertas("sol-1", demanda, AGORA)
    bases_ofertadas = {o["base_operacional_id"] for o in dados.buscar_ofertas_solicitacao("sol-1")}
...
def test_aceites_concorrentes_produzem_um_unico_vencedor(tmp_path, monkeypatch):
    ...
    assert (
        dados.conn.execute("SELECT COUNT(*) FROM oferta_coleta WHERE status = 'ACEITA'").fetchone()[
            0
        ]
        == 1
    )
